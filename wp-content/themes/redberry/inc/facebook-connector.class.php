  ...
  private function registerNewUser($exclude_fields) {
    if (is_array($this->reg_results)) {
      ...
      if(is_wp_error($insert_user_result)) {
        ...
        return array(
          'reg_status' => false,
          'msg' => __('There are some validation problems. Please, try again', 'redberry')
          );
      }
      elseif ($insert_user_result > 0) {
        ...
        return array(
          'reg_status' => true,
          'msg' => __('Your new account created successfully', 'redberry')
          );
      }
    }
    return array(
      'reg_status' => false,
      'msg' => __('For registration we need correct data. Please, try again', 'redberry')
      );
  }
